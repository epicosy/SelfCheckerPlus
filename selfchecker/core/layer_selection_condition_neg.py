...
def selected_layer_for_label(num_layers, num_classes, layers_agree, label, selected_layers_dict,
                             weights_dict, working_dir: Path):
...
def layer_selection_condition_neg(num_layers, labels: list, working_dir: Path):
    ...
    for label in labels:
        ...
        with (working_dir / f"selected_layers_accuracy_neg_{label}.json").open(mode='w') as json_file:
            json.dump(selected_layers_dict, json_file, ensure_ascii=False)

        with (working_dir / f"weights_neg_{label}.json").open(mode='w') as json_file:
